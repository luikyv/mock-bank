...
func PermissionMiddleware(next http.Handler, consentService Service, permissions ...Permission) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			apiutil.WriteError(w, apiutil.NewError("UNAUTHORISED", http.StatusUnauthorized, "invalid token"))
			return
		}

		if !consent.IsAuthorized() {
			slog.DebugContext(r.Context(), "the consent is not authorized")
			apiutil.WriteError(w, apiutil.NewError("INVALID_STATUS", http.StatusUnauthorized, "the consent is not authorized"))
			return
		}

		if !consent.HasPermissions(permissions) {
			slog.DebugContext(r.Context(), "the consent doesn't have the required permissions")
			apiutil.WriteError(w, apiutil.NewError("INVALID_STATUS", http.StatusForbidden, "the consent is missing permissions"))
		}
