...
func (p paymentPolicy) grantPayment(w http.ResponseWriter, r *http.Request, as *goidc.AuthnSession) (goidc.AuthnStatus, error) {
	...
	if isConsented == "" {
		...
		page := map[string]any{
			...
			"CreditorCPFCNPJ":       c.Creditor.CPFCNPJ,
			"CreditorAccountISBP":   c.CreditorAccount.ISBP,
			"CreditorAccountIssuer": c.CreditorAccount.Issuer,
			"CreditorAccountNumber": c.CreditorAccount.Number,
