...
func seedRalphBragg(ctx context.Context, db *gorm.DB) error {
	...
	for i, transactionID := range []string{
		...
		testTransaction := &account.Transaction{
			...
			Amount:           "771.52",
			PartieBranchCode: pointerOf("6272"),
			PartieCheckDigit: pointerOf("4"),
			PartieCNPJCPF:    pointerOf("87517400444"),
			PartieCompeCode:  pointerOf("123"),
			PartieNumber:     pointerOf("94088392"),
			PartiePersonType: pointerOf(account.PersonTypeIndividual),
			OrgID:            OrgID,
			CrossOrg:         true,
			UpdatedAt:        timeutil.DateTimeNow(),
		}
