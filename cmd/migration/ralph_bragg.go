...
func seedRalphBragg(ctx context.Context, db *gorm.DB) error {
	...
	for i, transactionID := range []string{
		...
		testTransaction := &account.Transaction{
			...
			Amount:           "771.52",
			PartieBranchCode: runutil.PointerOf("6272"),
			PartieCheckDigit: runutil.PointerOf("4"),
			PartieCNPJCPF:    runutil.PointerOf("87517400444"),
			PartieCompeCode:  runutil.PointerOf("123"),
			PartieNumber:     runutil.PointerOf("94088392"),
			PartiePersonType: runutil.PointerOf(account.PersonTypeIndividual),
			OrgID:            OrgID,
			CrossOrg:         true,
			UpdatedAt:        timeutil.DateTimeNow(),
		}
	...
	testLoan := &creditop.Contract{
		...
		ProductSubTypeCategory:              runutil.PointerOf(creditop.ProductSubTypeCategoryPersonal),
		Date:                                timeutil.NewBrazilDate(time.Date(2022, 1, 8, 12, 0, 0, 0, time.UTC)),
		DisbursementDates:                   runutil.PointerOf([]timeutil.BrazilDate{timeutil.NewBrazilDate(time.Date(2022, 1, 8, 12, 0, 0, 0, time.UTC))}),
		SettlementDate:                      runutil.PointerOf(timeutil.NewBrazilDate(time.Date(2021, 6, 21, 12, 0, 0, 0, time.UTC))),
		Amount:                              "12070.60",
		Currency:                            runutil.PointerOf("BRL"),
		DueDate:                             runutil.PointerOf(timeutil.NewBrazilDate(time.Date(2023, 1, 8, 12, 0, 0, 0, time.UTC))),
		InstalmentPeriodicity:               creditop.PeriodicityIrregular,
		InstalmentPeriodicityAdditionalInfo: runutil.PointerOf("DIA"),
		NextInstalmentAmount:                runutil.PointerOf("100.00"),
		FirstInstalmentDueDate:              runutil.PointerOf(timeutil.NewBrazilDate(time.Date(2022, 1, 8, 12, 0, 0, 0, time.UTC))),
		CET:                                 "0.015000",
		AmortizationSchedule:                creditop.AmortizationSchedulePRICE,
		AmortizationScheduleAdditionalInfo:  runutil.PointerOf("NA"),
		CNPJConsignee:                       runutil.PointerOf("13832718000196"),
		InterestRates: []creditop.InterestRate{{
			...
			RateIndexerSubType:        runutil.PointerOf(creditop.RateIndexerSubTypeFixed),
			RateIndexerAdditionalInfo: nil,
			FixedRate:                 runutil.PointerOf("0.015000"),
			PostFixedRate:             runutil.PointerOf("0.000000"),
			AdditionalInfo:            runutil.PointerOf("NA"),
		}},
		ContractedFees: []creditop.Fee{{
			...
			Amount:            runutil.PointerOf("200.50"),
			Rate:              runutil.PointerOf("0.000000"),
		}},
		FinanceCharges: []creditop.FinanceCharge{{
			Type:           creditop.FinanceChargeTypeLatePaymentFine,
			AdditionalInfo: runutil.PointerOf("NA"),
			Rate:           runutil.PointerOf("0.060000"),
		}},
		OutstandingBalance:      "14402.37",
		PaidInstalments:         runutil.PointerOf(3),
		DueInstalments:          730,
		PastDueInstalments:      727,
		TotalInstalments:        runutil.PointerOf(1),
		TotalInstalmentType:     creditop.InstalmentPeriodicityTotalDay,
		RemainingInstalments:    runutil.PointerOf(727),
	...
	testLoanReleasePayment := &creditop.ReleasePayment{
		...
		InstalmentID:        runutil.PointerOf("6bb40f5a-23e4-4c46-a2a4-c287ec72c0ac"),
